...
static btScalar sGripperClosingTargetVelocity = (btScalar)0.f;
...
public:
    ...
    void stepSimulation(btScalar deltaTime)
    {
        ...
        for (int i = 0; i < num_multiBody; ++i)
        {
            ...
            mb->setBaseVel(btVector3((btScalar)0,sGripperVerticalVelocity, (btScalar)0));
            ...
            for (int link = 0; link < mb->getNumLinks(); link++)
            {
                if (supportsJointMotor(mb, link))
                {
                    ...
                    if (motor)
                    {
                        if (dofIndex == 6)
                        {
                            motor->setVelocityTarget(-fingerTargetVelocities[1], (btScalar)1);
                            motor->setMaxAppliedImpulse((btScalar)20);
                        }
                        if (dofIndex == 7)
                        {
                            motor->setVelocityTarget(fingerTargetVelocities[1], (btScalar)1);
                        ...
                        motor->setMaxAppliedImpulse((btScalar)1);
                    }
                }
...
void GraspDeformable::initPhysics()
{
    ...
    getDeformableDynamicsWorld()->getSolverInfo().m_deformable_erp = (btScalar)0.1;
    getDeformableDynamicsWorld()->getSolverInfo().m_deformable_cfm = (btScalar)0;
...
//        psb->translate(btVector3(.25, 10, 0.4));
        ...
        btDeformableNeoHookeanForce* neohookean = new btDeformableNeoHookeanForce((btScalar)20, (btScalar)80, (btScalar).01);
    ...
    {
        bool onGround = false;
        const btScalar s = (btScalar).05;
        const btScalar h = (btScalar)-0.02;
        ...
        psb->getCollisionShape()->setMargin((btScalar)0.001);
        psb->generateBendingConstraints(2);
        psb->setTotalMass((btScalar)0.01);
        psb->setSpringStiffness((btScalar)10);
        psb->setDampingCoefficient((btScalar)0.05);
        psb->m_cfg.kKHR = (btScalar)1; // collision hardness with kinematic objects
        psb->m_cfg.kCHR = (btScalar)1; // collision hardness with rigid body
        ...
        psb->m_cfg.collisions |= btSoftBody::fCollision::SDF_RDF;
        getDeformableDynamicsWorld()->addSoftBody(psb);
        getDeformableDynamicsWorld()->addForce(psb, new btDeformableMassSpringForce((btScalar)0.05, (btScalar)0.005, true));
        getDeformableDynamicsWorld()->addForce(psb, new btDeformableGravityForce(gravity* (btScalar)0.1));
    }
...
btMultiBody* GraspDeformable::createFeatherstoneMultiBody(btMultiBodyDynamicsWorld* pWorld, const btVector3& basePosition, const btVector3& baseHalfExtents, const btVector3& linkHalfExtents, bool floating)
{
    ...
    parentComToCurrentCom.push_back(btVector3((btScalar)0, -linkHalfExtents[1] * (btScalar)8.f, -baseHalfExtents[2] * (btScalar)2.f));
    parentComToCurrentCom.push_back(btVector3((btScalar)0, -linkHalfExtents[1] * (btScalar)8.f, baseHalfExtents[2] * (btScalar)2.f));//par body's COM to cur body's COM offset
    
    btVector3 currentPivotToCurrentCom((btScalar)0, -linkHalfExtents[1]* (btScalar)8.f, (btScalar)0);                         //cur body's COM to cur body's PIV offset
    ...
    for (int i = 0; i < numLinks; ++i)
    {
        pMultiBody->setupRevolute(i, (btScalar)linkMass, linkInertiaDiag, - 1, btQuaternion((btScalar)0.f, (btScalar)0.f, (btScalar)0.f, (btScalar)1.f), hingeJointAxis, parentComToCurrentPivot[i], currentPivotToCurrentCom, true);
    }
