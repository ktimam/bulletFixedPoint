...
void btRaycastVehicle::updateFriction(btScalar timeStep)
{
	...
	btScalar fwdFactor = (btScalar)0.5;
	...
	bool sliding = false;
	{
		for (int wheel = 0; wheel < getNumWheels(); wheel++)
		{
			...
			btScalar rollingFriction = (btScalar)0.f;

			if (groundObject)
			{
				if (wheelInfo.m_engineForce != (btScalar)0.f)
				{
					rollingFriction = wheelInfo.m_engineForce * timeStep;
				}
				else
				{
					btScalar defaultRollingFrictionImpulse = (btScalar)0.f;
	...
	// apply the impulses
	{
		for (int wheel = 0; wheel < getNumWheels(); wheel++)
		{
			...
			if (m_sideImpulse[wheel] != btScalar(0.))
			{
				...
				rel_pos -= vChassisWorldUp * (vChassisWorldUp.dot(rel_pos) * ((btScalar)1.f - wheelInfo.m_rollInfluence));
#else
