...
btScalar btMachEps()
{
	...
	if (!calculated)
	{
		...
		} while ((btScalar)((btScalar)1.0 + (machEps / btScalar(2.0))) != btScalar(1.0));
...
btScalar btEpsRoot()
{
	static btScalar epsroot = (btScalar)0.;
...
btVectorXu btLemkeAlgorithm::solve(unsigned int maxloops /* = 0*/)
{
	...
	int pivotRowIndex = -1;
	btScalar minValue = (btScalar)1e30f;
...
int btLemkeAlgorithm::findLexicographicMinimum(const btMatrixXu& A, const int& pivotColIndex, const int& z0Row, bool& isRayTermination)
{
	...
	btAlignedObjectArray<int> activeRows;
        ...
        bool firstRow = true;
	btScalar currentMin = (btScalar)0.0;
	...
	for (int row = 0; row < dim; row++)
	{
		...
		if (denom > btMachEps())
		{
			...
			else if (abs(currentMin - q) < btMachEps())
			{
				activeRows.push_back(row);
			}
	...
	for (int col = 0; col < dim ; col++)
	{
		...
		for (int i = 0; i<activeRowsCopy.size();i++)
		{
			...
			else if (abs(currentMin - ratio) < btMachEps())
			{
				activeRows.push_back(row);
			}
...
void btLemkeAlgorithm::GaussJordanEliminationStep(btMatrixXu& A, int pivotRowIndex, int pivotColumnIndex, const btAlignedObjectArray<int>& basis)
{
	...
	for (int i = 0; i < A.rows(); i++)
	{
		if (i != pivotRowIndex)
		{
			A.setElem(i, pivotColumnIndex, (btScalar)0);
		}
	}
#ifdef BT_DEBUG_OSTREAM
	cout << A << std::endl;
