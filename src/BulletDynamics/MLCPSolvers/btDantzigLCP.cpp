					   ...
					   btScalar *outer_w, int nub, btScalar *lo, btScalar *hi, int *findex, btDantzigScratchMemory &scratchMem)
{
...
#ifdef BT_DEBUG
	{
		...
		for (int k = 0; k < n; ++k)
			btAssert(lo[k] <= (btScalar)0 && hi[k] >= (btScalar)0);
	}
#endif
	...
	for (int i = adj_nub; i < n; ++i)
	{
		...
		if (lo[i] == (btScalar)0 && w[i] >= (btScalar)0)
		{
			lcp.transfer_i_to_N(i);
			scratchMem.state[i] = false;
		}
		else if (hi[i] == (btScalar)0 && w[i] <= (btScalar)0)
		{
			lcp.transfer_i_to_N(i);
			scratchMem.state[i] = true;
		}
		else if (w[i] == (btScalar)0)
		{
			...
			for (;;)
			{
...

				{
					...
					for (int k = 0; k < numN; ++k)
					{
						...
						if (!scratchMem.state[indexN_k] ? scratchMem.delta_w[indexN_k] < (btScalar)0 : scratchMem.delta_w[indexN_k] > (btScalar)0)
						{
							// don't bother checking if lo=hi=0
							if (lo[indexN_k] == (btScalar)0 && hi[indexN_k] == (btScalar)0) continue;
...

				{
					...
					for (int k = adj_nub; k < numC; ++k)
					{
						...
						if (scratchMem.delta_x[indexC_k] < (btScalar)0 && lo[indexC_k] > -BT_INFINITY)
						{
						...
						if (scratchMem.delta_x[indexC_k] > (btScalar)0 && hi[indexC_k] < BT_INFINITY)
						{
