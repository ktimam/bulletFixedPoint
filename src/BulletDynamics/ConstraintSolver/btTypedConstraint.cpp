...
{
	m_halfRange = (high - low) / (btScalar)2.0f;
...
{
	m_correction = (btScalar)0.0f;
	m_sign = (btScalar)0.0f;
	m_solveLimit = false;

	if (m_halfRange >= (btScalar)0.0f)
	{
...
void btAngularLimit::fit(btScalar& angle) const
{
	if (m_halfRange > (btScalar)0.0f)
	{
		...
		{
			if (relativeAngle > (btScalar)0.0f)
			{
				angle = getHigh();
			}
			else
