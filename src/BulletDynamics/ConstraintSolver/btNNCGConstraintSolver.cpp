...
btScalar btNNCGConstraintSolver::solveSingleIteration(int iteration, btCollisionObject** /*bodies */, int /*numBodies*/, btPersistentManifold** /*manifoldPtr*/, int /*numManifolds*/, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* /*debugDrawer*/)
{
	...
	btScalar deltaflengthsqr = (btScalar)0;
	{
	...
	if (m_onlyForNoneContact)
	{
		...
		else
		{
			...
			btScalar beta = m_deltafLengthSqrPrev > (btScalar)0 ? deltaflengthsqr / m_deltafLengthSqrPrev : (btScalar)2;
			if (beta > (btScalar)1)
			{
				for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++) m_pNC[j] = (btScalar)0;
			}
			else
			{
				for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++)
					...
					{
						{
							...
							{
								m_deltafCF[c * multiplier] = (btScalar)0;
							}
						}
						...
						if (infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS)
						{
							...
							{
								m_deltafCF[c * multiplier + 1] = (btScalar)0;
							}
						}
					}
				}
	...
	if (!m_onlyForNoneContact)
	{
		...
		else
		{
			...
			{
				for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++) m_pNC[j] = (btScalar)0;
				for (int j = 0; j < m_tmpSolverContactConstraintPool.size(); j++) m_pC[j] = (btScalar)0;
				for (int j = 0; j < m_tmpSolverContactFrictionConstraintPool.size(); j++) m_pCF[j] = (btScalar)0;
				for (int j = 0; j < m_tmpSolverContactRollingFrictionConstraintPool.size(); j++) m_pCRF[j] = (btScalar)0;
			}
			else
			{
				for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++)
