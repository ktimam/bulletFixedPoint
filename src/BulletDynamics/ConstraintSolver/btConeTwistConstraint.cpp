...
void btConeTwistConstraint::getInfo2NonVirtual(btConstraintInfo2* info, const btTransform& transA, const btTransform& transB, const btMatrix3x3& invInertiaWorldA, const btMatrix3x3& invInertiaWorldB)
{
	...
	// set jacobian
	info->m_J1linearAxis[0] = (btScalar)1;
	info->m_J1linearAxis[info->rowskip + 1] = (btScalar)1;
	info->m_J1linearAxis[2 * info->rowskip + 2] = (btScalar)1;
	...
	info->m_J2linearAxis[0] = (btScalar)-1;
	info->m_J2linearAxis[info->rowskip + 1] = (btScalar)-1;
	info->m_J2linearAxis[2 * info->rowskip + 2] = (btScalar)-1;
	...
	for (j = 0; j < 3; j++)
	{
		...
		info->m_lowerLimit[j * info->rowskip] = (btScalar)-SIMD_INFINITY;
		info->m_upperLimit[j * info->rowskip] = (btScalar)SIMD_INFINITY;
	...
	if (m_solveSwingLimit)
	{
		...
		if ((m_swingSpan1 < m_fixThresh) && (m_swingSpan2 < m_fixThresh))
		{
			...
			info->m_lowerLimit[srow1] = (btScalar)-SIMD_INFINITY;
			info->m_upperLimit[srow1] = (btScalar)SIMD_INFINITY;
		...
		else
		{
			...
			info->m_lowerLimit[srow] = (btScalar)0;
			info->m_upperLimit[srow] = (m_bMotorEnabled && m_maxMotorImpulse >= (btScalar)0.0f) ? m_maxMotorImpulse : (btScalar)SIMD_INFINITY;
			srow += info->rowskip;
		}
	}
	if (m_solveTwistLimit)
	{
		...
		if (m_twistSpan > (btScalar)0.0f)
		{
			if (m_twistCorrection > (btScalar)0.0f)
			{
				info->m_lowerLimit[srow] = (btScalar)0;
				info->m_upperLimit[srow] = (btScalar)SIMD_INFINITY;
			}
			...
			{
				info->m_lowerLimit[srow] = (btScalar)-SIMD_INFINITY;
				info->m_upperLimit[srow] = (btScalar)0;
			}
		}
		else
		{
...
void btConeTwistConstraint::buildJacobian()
{
	if (m_useSolveConstraintObsolete)
	{
		...
		if (!m_angularOnly)
		{
			...
			{
				normal[0].setValue(btScalar(1.0), (btScalar)0, (btScalar)0);
			}
...
void btConeTwistConstraint::solveConstraintObsolete(btSolverBody& bodyA, btSolverBody& bodyB, btScalar timeStep)
{
	...
	if (m_useSolveConstraintObsolete)
	{
		...
		if (m_bMotorEnabled)
		{
			...
			btScalar kAxisAInv = (btScalar)0, kAxisBInv = (btScalar)0;
			...
			if (bDoTorque && avgAxis.length2() > (btScalar)SIMD_EPSILON)
			{
				...
				if (m_maxMotorImpulse >= (btScalar)0)
				{
		...
		// joint limits
		{
			...
			if (m_solveTwistLimit)
			{
				...
				if (relTwistVel > (btScalar)0)  // only damp when moving towards limit (m_twistAxis flipping is important)
...
void btConeTwistConstraint::calcAngleInfo()
{
	...
	btScalar RMaxAngle1Sq = (btScalar)1.0f / (m_swingSpan1 * m_swingSpan1);
	btScalar RMaxAngle2Sq = (btScalar)1.0f / (m_swingSpan2 * m_swingSpan2);
	btScalar EllipseAngle = btFabs(swing1 * swing1) * RMaxAngle1Sq + btFabs(swing2 * swing2) * RMaxAngle2Sq;

	if (EllipseAngle > (btScalar)1.0f)
	{
		m_swingCorrection = EllipseAngle - (btScalar)1.0f;
		m_solveSwingLimit = true;
		// Calculate necessary axis & factors
		m_swingAxis = b2Axis1.cross(b1Axis2 * b2Axis1.dot(b1Axis2) + b1Axis3 * b2Axis1.dot(b1Axis3));
		m_swingAxis.normalize();
		btScalar swingAxisSign = (b2Axis1.dot(b1Axis1) >= (btScalar)0.0f) ? (btScalar)1.0f : (btScalar)-1.0f;
	...
	if (m_twistSpan >= btScalar(0.))
	{
		...
		if (twist <= -m_twistSpan * lockedFreeFactor)
		{
			...
			m_twistAxis = (b2Axis1 + b1Axis1) * (btScalar)0.5f;
			m_twistAxis.normalize();
			m_twistAxis *= (btScalar)-1.0f;
		}
...
void btConeTwistConstraint::calcAngleInfo2(const btTransform& transA, const btTransform& transB, const btMatrix3x3& invInertiaWorldA, const btMatrix3x3& invInertiaWorldB)
{
...

	{
		...
		if (m_swingSpan1 >= m_fixThresh && m_swingSpan2 >= m_fixThresh)
		{
			btScalar swingAngle, swingLimit = (btScalar)0;
			btVector3 swingAxis;
			...
			if (swingAngle > swingLimit * m_limitSoftness)
			{
				...
				// 1 == hard/real limit
				m_swingLimitRatio = (btScalar)1.f;
				if (swingAngle < swingLimit && m_limitSoftness < (btScalar)1.f - (btScalar)SIMD_EPSILON)
				{
		...
		if (m_twistSpan >= btScalar(0.f))
		{
			...
			if (m_twistAngle > m_twistSpan * m_limitSoftness)
			{
				...
				m_twistLimitRatio = (btScalar)1.f;
				if (m_twistAngle < m_twistSpan && m_limitSoftness < (btScalar)1.f - (btScalar)SIMD_EPSILON)
				{
												 ...
												 btScalar& swingLimit)   // out
{
	...
	if (swingAngle > (btScalar)SIMD_EPSILON)
	{
       ...
       btAssert(fabs(vSwingAxis.x()) <= SIMD_EPSILON));
		...
		if (abs(xEllipse) > (btScalar)SIMD_EPSILON)
		{
...
btVector3 btConeTwistConstraint::GetPointForAngle(btScalar fAngleInRadians, btScalar fLength) const
{
	...
	btVector3 vSwingAxis((btScalar)0, xEllipse, -yEllipse);
	btQuaternion qSwing(vSwingAxis, swingLimit);
	btVector3 vPointInConstraintSpace(fLength, (btScalar)0, (btScalar)0);
...
void btConeTwistConstraint::adjustSwingAxisToUseEllipseNormal(btVector3& vSwingAxis) const
{
	...
	// do the math...
	if (abs(z) > (btScalar)SIMD_EPSILON)  // avoid division by 0. and we don't need an update if z == 0.
	{
...
void btConeTwistConstraint::setMotorTargetInConstraintSpace(const btQuaternion& q)
{
	...
	{
		btScalar softness = (btScalar)1.f;  //m_limitSoftness;
		...
		if (m_swingSpan1 >= btScalar(0.05f) && m_swingSpan2 >= btScalar(0.05f))
		{
			...
			if (abs(swingAngle) > (btScalar)SIMD_EPSILON)
			{
		...
		if (m_twistSpan >= btScalar(0.05f))
		{
			...
			if (abs(twistAngle) > (btScalar)SIMD_EPSILON)
			{
