...
#define D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION 1.0e-3
			bool indx1Violated = m_angularLimits[indx1].m_currentLimit == 1 ||
								 ...
								 (m_angularLimits[indx1].m_currentLimit == 3 && (m_angularLimits[indx1].m_currentLimitError < (btScalar)-D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION || m_angularLimits[indx1].m_currentLimitError >(btScalar)D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION)) ||
								 (m_angularLimits[indx1].m_currentLimit == 4 && (m_angularLimits[indx1].m_currentLimitError < (btScalar)-D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION || m_angularLimits[indx1].m_currentLimitErrorHi >(btScalar)D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION));
			bool indx2Violated = m_angularLimits[indx2].m_currentLimit == 1 ||
								 m_angularLimits[indx2].m_currentLimit == 2 ||
								 (m_angularLimits[indx2].m_currentLimit == 3 && (m_angularLimits[indx2].m_currentLimitError < (btScalar)-D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION || m_angularLimits[indx2].m_currentLimitError >(btScalar)D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION)) ||
								 (m_angularLimits[indx2].m_currentLimit == 4 && (m_angularLimits[indx2].m_currentLimitError < (btScalar)-D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION || m_angularLimits[indx2].m_currentLimitErrorHi >(btScalar)D6_LIMIT_ERROR_THRESHOLD_FOR_ROTATION));
	...
	btConstraintInfo2* info, int row, btVector3& ax1, int rotational, int rotAllowed)
{
	...
	if (limot->m_currentLimit == 4)
	{
		...
		if (rotational)
		{
			if (info->m_constraintError[srow] - vel * limot->m_stopERP > (btScalar)0)
			{
		...
		info->m_lowerLimit[srow] = rotational ? (btScalar)0 : (btScalar)-SIMD_INFINITY;
		info->m_upperLimit[srow] = rotational ? (btScalar)SIMD_INFINITY : (btScalar)0;
		...
		info->m_lowerLimit[srow] = rotational ? (btScalar)-SIMD_INFINITY : (btScalar)0;
		info->m_upperLimit[srow] = rotational ? (btScalar)0 : (btScalar)SIMD_INFINITY;
		info->cfm[srow] = limot->m_stopCFM;
		srow += info->rowskip;
		++count;
	}
	else if (limot->m_currentLimit == 3)
	{
		calculateJacobi(limot, transA, transB, info, srow, ax1, rotational, rotAllowed);
		info->m_constraintError[srow] = info->fps * limot->m_stopERP * limot->m_currentLimitError * (rotational ? -1 : 1);
		info->m_lowerLimit[srow] = (btScalar)-SIMD_INFINITY;
		info->m_upperLimit[srow] = (btScalar)SIMD_INFINITY;
		info->cfm[srow] = limot->m_stopCFM;
		srow += info->rowskip;
		++count;
	}
	...
	if (limot->m_enableMotor && limot->m_servoMotor)
	{
		...
		calculateJacobi(limot, transA, transB, info, srow, ax1, rotational, rotAllowed);
		btScalar targetvelocity = error < (btScalar)0 ? -limot->m_targetVelocity : limot->m_targetVelocity;
		...
		if (error != (btScalar)0)
		{
			...
			{
				lowLimit = error > (btScalar)0 ? curServoTarget : (btScalar)-SIMD_INFINITY;
				hiLimit = error < (btScalar)0 ? curServoTarget : (btScalar)SIMD_INFINITY;
			}
			else
			{
				lowLimit = error > (btScalar)0 && curServoTarget > limot->m_loLimit ? curServoTarget : limot->m_loLimit;
				hiLimit = error < (btScalar)0 && curServoTarget < limot->m_hiLimit ? curServoTarget : limot->m_hiLimit;
			}
	...
	if (limot->m_enableSpring)
	{
		...
		btScalar dt = (btScalar)BT_ONE / info->fps;
		...
		btScalar cfm = (btScalar)BT_ZERO;
		btScalar mA = (btScalar)BT_ONE / m_rbA.getInvMass();
		btScalar mB = (btScalar)BT_ONE / m_rbB.getInvMass();
		if (rotational)
		{
		...
		btScalar m;
		if (m_rbA.getInvMass() == (btScalar)0) m = mB; else
		if (m_rbB.getInvMass() == (btScalar)0) m = mA; else
			m = mA*mB / (mA + mB);
		btScalar angularfreq = btSqrt(ks / m);

		//limit stiffness (the spring should not be sampled faster that the quarter of its angular frequency)
		if (limot->m_springStiffnessLimited && (btScalar)0.25 < angularfreq * dt)
		{
			ks = (btScalar)BT_ONE / dt / dt / btScalar(16.0) * m;
		}
		...
		if (m_flags & BT_6DOF_FLAGS_USE_INFINITE_ERROR)
			info->m_constraintError[srow] = (rotational ? -1 : 1) * (f < (btScalar)0 ? (btScalar)-SIMD_INFINITY : (btScalar)SIMD_INFINITY);
		else
		...
		if (!rotational)
		{
			info->m_lowerLimit[srow] = minf > (btScalar)0 ? (btScalar)0 : minf;
			info->m_upperLimit[srow] = maxf < (btScalar)0 ? (btScalar)0 : maxf;
		}
		else
		{
			info->m_lowerLimit[srow] = -maxf > (btScalar)0 ? (btScalar)0 : -maxf;
			info->m_upperLimit[srow] = -minf < (btScalar)0 ? (btScalar)0 : -minf;
		}
...
void btTranslationalLimitMotor2::testLimitValue(int limitIndex, btScalar test_value)
{
	...
	if (loLimit > hiLimit)
	{
		m_currentLimitError[limitIndex] = (btScalar)0;
