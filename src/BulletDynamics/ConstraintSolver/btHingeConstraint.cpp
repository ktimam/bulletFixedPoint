	  ...
	  m_stopERP(0)
{
	...
	if (projection >= (btScalar)1.0f - (btScalar)SIMD_EPSILON)
	{
		rbAxisA1 = -rbA.getCenterOfMassTransform().getBasis().getColumn(2);
		rbAxisA2 = rbA.getCenterOfMassTransform().getBasis().getColumn(1);
	}
	else if (projection <= (btScalar)-1.0f + (btScalar)SIMD_EPSILON)
	{
		rbAxisA1 = rbA.getCenterOfMassTransform().getBasis().getColumn(2);
		rbAxisA2 = rbA.getCenterOfMassTransform().getBasis().getColumn(1);
	}
...
void btHingeConstraint::buildJacobian()
{
	if (m_useSolveConstraintObsolete)
	{
		...
		m_kHinge = (btScalar)1.0f / (getRigidBodyA().computeAngularImpulseDenominator(axisA) +
...
{
	return btFmod(btFmod(angle, btScalar((btScalar)2.0 * SIMD_PI)) + btScalar((btScalar)2.0 * SIMD_PI), btScalar((btScalar)2.0 * SIMD_PI));
}
...
void btHingeConstraint::getInfo2Internal(btConstraintInfo2* info, const btTransform& transA, const btTransform& transB, const btVector3& angVelA, const btVector3& angVelB)
{
	...
	if (!m_angularOnly)
	{
		...
		info->m_J1linearAxis[skip + 1] = (btScalar)1;
		info->m_J1linearAxis[2 * skip + 2] = (btScalar)1;

		info->m_J2linearAxis[0] = (btScalar)-1;
		info->m_J2linearAxis[skip + 1] = (btScalar)-1;
		info->m_J2linearAxis[2 * skip + 2] = (btScalar)-1;
	}
	...
	if (limit || powered)
	{
		...
		if (limit)
		{
			...
			{  // high limit
				info->m_lowerLimit[srow] = (btScalar)(btScalar)-SIMD_INFINITY;
...
static btVector3 vHinge((btScalar)0, (btScalar)0, btScalar(1));
...
//	btVector3 pivotBInW = trB.getOrigin();
	...
	if (ax1.length2()< (btScalar)SIMD_EPSILON)
	{
		factA= (btScalar)0.f;
		factB= (btScalar)1.f;
