...
public:
	...
	{
		m_loLimit = (btScalar)1.0f;
		m_hiLimit = (btScalar)-1.0f;
		m_bounce = (btScalar)0.0f;
		m_stopERP = (btScalar)0.2f;
		m_stopCFM = (btScalar)0.f;
		m_motorERP = (btScalar)0.9f;
		m_motorCFM = (btScalar)0.f;
		m_enableMotor = false;
		m_targetVelocity = (btScalar)0;
		m_maxMotorForce = (btScalar)6.0f;
		m_servoMotor = false;
		m_servoTarget = (btScalar)0;
		m_enableSpring = false;
		m_springStiffness = (btScalar)0;
		m_springStiffnessLimited = false;
		m_springDamping = (btScalar)0;
		m_springDampingLimited = false;
		m_equilibriumPoint = (btScalar)0;

		m_currentLimitError = (btScalar)0;
		m_currentLimitErrorHi = (btScalar)0;
		m_currentPosition = (btScalar)0;
		m_currentLimit = 0;
	}
...
SIMD_FORCE_INLINE const char* btGeneric6DofSpring2Constraint::serialize(void* dataBuffer, btSerializer* serializer) const
{
	...
	dof->m_angularLowerLimit.m_floats[3] = (btScalar)0;
	dof->m_angularUpperLimit.m_floats[3] = (btScalar)0;
	dof->m_angularBounce.m_floats[3] = (btScalar)0;
	dof->m_angularStopERP.m_floats[3] = (btScalar)0;
	dof->m_angularStopCFM.m_floats[3] = (btScalar)0;
	dof->m_angularMotorERP.m_floats[3] = (btScalar)0;
	dof->m_angularMotorCFM.m_floats[3] = (btScalar)0;
	dof->m_angularTargetVelocity.m_floats[3] = (btScalar)0;
	dof->m_angularMaxMotorForce.m_floats[3] = (btScalar)0;
	dof->m_angularServoTarget.m_floats[3] = (btScalar)0;
	dof->m_angularSpringStiffness.m_floats[3] = (btScalar)0;
	dof->m_angularSpringDamping.m_floats[3] = (btScalar)0;
	dof->m_angularEquilibriumPoint.m_floats[3] = (btScalar)0;
	for (i = 0; i < 4; i++)
	{
