...
{
	btScalar deltaImpulse = (btScalar)0.f;
	...
	const btVector3& rel_pos1, const btVector3& rel_pos2)
{
	...
	else
	{
		if (cp.m_contactPointFlags & BT_CONTACT_FLAG_CONTACT_STIFFNESS_DAMPING)
		{
			...
			if (denom < (btScalar)SIMD_EPSILON)
			{
				denom = (btScalar)SIMD_EPSILON;
			}
...

	{
		...
		if (penetration > (btScalar)0)
		{
			positionalError = (btScalar)0;
		...
		solverConstraint.m_lowerLimit = (btScalar)0;
...
{
	{
		...
		frictionConstraint1.m_appliedImpulse = (btScalar)0.f;
	}
	...
	if ((infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS))
	{
		...
		frictionConstraint2.m_appliedImpulse = (btScalar)0.f;
	}
}
...
void btSequentialImpulseConstraintSolver::convertContact(btPersistentManifold* manifold, const btContactSolverInfo& infoGlobal)
{
	...
	for (int j = 0; j < manifold->getNumContacts(); j++)
	{
		...
		if (cp.getDistance() <= manifold->getContactProcessingThreshold())
		{
			...
			if ((cp.m_combinedRollingFriction > (btScalar)0.f) && (rollingFriction > 0))
			{
				{
			...
			if (!(infoGlobal.m_solverMode & SOLVER_ENABLE_FRICTION_DIRECTION_CACHING) || !(cp.m_contactPointFlags & BT_CONTACT_FLAG_LATERAL_FRICTION_INITIALIZED))
			{
				...
				if (!(infoGlobal.m_solverMode & SOLVER_DISABLE_VELOCITY_DEPENDENT_FRICTION_DIRECTION) && lat_rel_vel > (btScalar)SIMD_EPSILON)
				{
	...
	const btContactSolverInfo& infoGlobal)
{
	...
	for (int j = 0; j < info1.m_numConstraintRows; j++)
	{
		...
		currentConstraintRow[j].m_lowerLimit = (btScalar)-SIMD_INFINITY;
		currentConstraintRow[j].m_upperLimit = (btScalar)SIMD_INFINITY;
		currentConstraintRow[j].m_appliedImpulse = (btScalar)0.f;
		currentConstraintRow[j].m_appliedPushImpulse = (btScalar)0.f;
	...
	info2.fps = (btScalar)1.f / infoGlobal.m_timeStep;
	...
	for (int j = 0; j < info1.m_numConstraintRows; j++)
	{
...

		{
			...
			btAssert(fsum > (btScalar)SIMD_EPSILON);
			btScalar sorRelaxation = (btScalar)1.f;  //todo: get from globalInfo?
			solverConstraint.m_jacDiagABInv = fsum > (btScalar)SIMD_EPSILON ? sorRelaxation / sum : (btScalar)0.f;
		}
...
btScalar btSequentialImpulseConstraintSolver::solveSingleIteration(int iteration, btCollisionObject** /*bodies */, int /*numBodies*/, btPersistentManifold** /*manifoldPtr*/, int /*numManifolds*/, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* /*debugDrawer*/)
{
	...
	if (iteration < infoGlobal.m_numIterations)
	{
		...
		if (infoGlobal.m_solverMode & SOLVER_INTERLEAVE_CONTACT_AND_FRICTION_CONSTRAINTS)
		{
			...
			{
				btScalar totalImpulse = (btScalar)0;
