...
struct btContactSolverInfo : public btContactSolverInfoData
{
	inline btContactSolverInfo()
	{
		...
		m_sor = btScalar(1.);
		m_splitImpulse = true;
		m_splitImpulsePenetrationThreshold = (btScalar)-.04f;
		m_splitImpulseTurnErp = (btScalar)0.1f;
		...
		m_maxGyroscopicForce = (btScalar)100.f;                          ///it is only used for 'explicit' version of gyroscopic force
		m_singleAxisRollingFrictionThreshold = (btScalar)1e30f;          ///if the velocity is above this threshold, it will use a single constraint row (axis), otherwise 3 rows.
		m_leastSquaresResidualThreshold = (btScalar)0.f;
		m_restitutionVelocityThreshold = (btScalar)0.2f;  //if the relative velocity is below this threshold, there is zero restitution
