													 ...
													 const btContactSolverInfo& infoGlobal)
{
	...
	jacobianA(0)[offsetA] = (btScalar)1;
	jacobianB(0)[offsetB] = m_gearRatio;

	btScalar posError = (btScalar)0;
	const btVector3 dummy(0, 0, 0);

	btScalar kp = (btScalar)1;
	btScalar kd = (btScalar)1;
	...
	for (int row = 0; row < numRows; row++)
	{
		...
		fillMultiBodyConstraint(constraintRow, data, jacobianA(row), jacobianB(row), dummy, dummy, dummy, dummy, posError, infoGlobal, -m_maxAppliedImpulse, m_maxAppliedImpulse, false, (btScalar)1, false, desiredRelativeVelocity);
