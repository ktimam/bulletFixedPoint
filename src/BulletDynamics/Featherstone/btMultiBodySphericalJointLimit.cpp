...
btMultiBodySphericalJointLimit::btMultiBodySphericalJointLimit(btMultiBody* body, int link, 
	...
	m_desiredPosition((btScalar)0, (btScalar)0, (btScalar)0, (btScalar)1),
...
void btMultiBodySphericalJointLimit::finalizeMultiDof()
{
	...
	// row 0: the lower bound
	jacobianA(0)[offset] = (btScalar)1;

	jacobianB(1)[offset] = (btScalar)-1;
												 ...
												 const btContactSolverInfo& infoGlobal)
{
	...
	const btScalar posError = (btScalar)0;
	...
	if (vTwistAxis.dot(vTwist)< (btScalar)0)
	...
	for (int row = 0; row < getNumRows(); row++)
	{
		...
		btScalar damp = (btScalar)1;
		if((angleDiff[row]>-allowed)&&(angleDiff[row]<allowed))
		{
			angleDiff[row]= (btScalar)0;
			damp= (btScalar)0;
		...
		btScalar posError = (btScalar)0;
		{
			...
			switch (m_bodyA->getLink(m_linkA).m_jointType)
			{
				case btMultibodyLink::eSpherical:
				{
					...
					btScalar kp = (btScalar)m_use_multi_dof_params ? m_kp[row % 3] : m_kp[0];
					posError = kp*angleDiff[row % 3];
					btScalar max_applied_impulse = (btScalar)m_use_multi_dof_params ? m_maxAppliedImpulseMultiDof[row % 3] : m_maxAppliedImpulse;
					...
					if (posError> (btScalar)0)
						max_applied_impulse= (btScalar)0;
					else
						min_applied_impulse= (btScalar)0;
					...
					if (btFabs(posError)>SIMD_EPSILON)
					{
						...
						fillMultiBodyConstraint(constraintRow, data, 0, 0, constraintNormalAng,
							...
							posError,
							infoGlobal,
							(btScalar)min_applied_impulse, (btScalar)max_applied_impulse, true,
							(btScalar)1.0, false, (btScalar)0, (btScalar)0,
							damp);
