														   ...
														   const btContactSolverInfo& infoGlobal)
{
	...
	for (int row = 0; row < getNumRows(); row++)
	{
		...
		btScalar direction = (btScalar)row ? (btScalar)-1 : (btScalar)1;
		...
		const btScalar posError = (btScalar)0;  //why assume it's zero?
		const btVector3 dummy(0, 0, 0);

		btScalar rel_vel = fillMultiBodyConstraint(constraintRow, data, jacobianA(row), jacobianB(row), dummy, dummy, dummy, dummy, posError, infoGlobal, (btScalar)0, m_maxAppliedImpulse);
