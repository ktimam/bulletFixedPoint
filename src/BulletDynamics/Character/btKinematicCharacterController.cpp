...
btKinematicCharacterController::btKinematicCharacterController(btPairCachingGhostObject* ghostObject, btConvexShape* convexShape, btScalar stepHeight, const btVector3& up)
{
	...
	setMaxSlope(btRadians((btScalar)45.0));
}
...
void btKinematicCharacterController::stepUp(btCollisionWorld* world)
{
	...
	if (m_verticalVelocity < (btScalar)0.0)
	...
	m_targetPosition = m_currentPosition + m_up * (stepHeight) + m_jumpAxis * ((m_verticalOffset > (btScalar)0.f ? m_verticalOffset : (btScalar)0.f));
	...
	if (callback.hasHit() && m_ghostObject->hasContactResponse() && needsCollision(m_ghostObject, callback.m_hitCollisionObject))
	{
		...
		if (callback.m_hitNormalWorld.dot(m_up) > (btScalar)0.0)
		{
		...
		if (m_verticalOffset > (btScalar)0)
		{
			m_verticalOffset = (btScalar)0.0;
			m_verticalVelocity = (btScalar)0.0;
...
void btKinematicCharacterController::stepDown(btCollisionWorld* collisionWorld, btScalar dt)
{
	...
	btScalar downVelocity = (m_verticalVelocity < (btScalar)0.f ? -m_verticalVelocity : (btScalar)0.f) * dt;

	if (m_verticalVelocity > (btScalar)0.0)
		return;

	if (downVelocity > (btScalar)0.0 && downVelocity > m_fallSpeed && (m_wasOnGround || !m_wasJumping))
	...
	while (1)
	{
		...
		btScalar downVelocity2 = (m_verticalVelocity < (btScalar)0.f ? -m_verticalVelocity : (btScalar)0.f) * dt;
		bool has_hit;
		if (bounce_fix == true)
		...
		if (downVelocity2 > (btScalar)0.0 && downVelocity2 < stepHeight && has_hit == true && runonce == false && (m_wasOnGround || !m_wasJumping))
		{
	...
	else
	{
		...
		if (bounce_fix == true)
		{
			downVelocity = (m_verticalVelocity < (btScalar)0.f ? -m_verticalVelocity : (btScalar)0.f) * dt;
...
void btKinematicCharacterController::setLinearVelocity(const btVector3& velocity)
{
	...
	if (m_walkDirection.length2() > (btScalar)0)
	{
		...
		if (c != (btScalar)0)
		{
			...
			m_verticalVelocity = (c < (btScalar)0.0f ? -1 : 1) * upComponent.length();
		...
		m_verticalVelocity = (btScalar)0.0f;
}
...
void btKinematicCharacterController::playerStep(btCollisionWorld* collisionWorld, btScalar dt)
{
	...
	if (m_AngVel.length2() > (btScalar)0.0f)
	{
	...
	// quick check...
	if (!m_useWalkDirection && (m_velocityTimeInterval <= (btScalar)0.0 || m_walkDirection.fuzzyZero()))
	{
		//		printf("\n");
		return;  // no motion
	}
	...
	if (m_verticalVelocity > (btScalar)0.0 && m_verticalVelocity > m_jumpSpeed)
	{
		m_verticalVelocity = m_jumpSpeed;
	}
	if (m_verticalVelocity < (btScalar)0.0 && btFabs(m_verticalVelocity) > btFabs(m_fallSpeed))
	{
...
{
	m_jumpSpeed = v.length2() == (btScalar)0 ? m_SetjumpSpeed : v.length();
	m_verticalVelocity = m_jumpSpeed;
	m_wasJumping = true;

	m_jumpAxis = v.length2() == (btScalar)0 ? m_up : v.normalized();
...
{
	if (gravity.length2() > (btScalar)0) setUpVector(-gravity);
...
{
	return (abs(m_verticalVelocity) < SIMD_EPSILON) && (abs(m_verticalOffset) < SIMD_EPSILON);
}
...
{
	if (up.length2() > (btScalar)0 && m_gravity > (btScalar)0.0f)
	{
