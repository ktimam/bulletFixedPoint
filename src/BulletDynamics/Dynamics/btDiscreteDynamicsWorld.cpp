			...
			if (getDispatchInfo().m_useContinuous && body->getCcdSquareMotionThreshold() && body->getCcdSquareMotionThreshold() < squareMotion)
			{
				...
				if (body->getCollisionShape()->isConvex())
				{
					...
					if (sweepResults.hasHit() && (sweepResults.m_closestHitFraction < (btScalar)1.f))
					{
						...
						pt.m_combinedRestitution = (btScalar)0;
	...
	if (m_applySpeculativeContactRestitution)
	{
		...
		for (int i = 0; i < m_predictiveManifolds.size(); i++)
		{
			...
			for (int p = 0; p < manifold->getNumContacts(); p++)
			{
				...
				if (combinedRestitution > (btScalar)0 && pt.m_appliedImpulse != (btScalar)0.f)
	...
	switch (constraint->getConstraintType())
	{
		...
		case SLIDER_CONSTRAINT_TYPE:
		{
			...
			if (drawLimits)
			{
				...
				btVector3 li_min = tr * btVector3(pSlider->getLowerLinLimit(), (btScalar)0.f, (btScalar)0.f);
				btVector3 li_max = tr * btVector3(pSlider->getUpperLinLimit(), (btScalar)0.f, (btScalar)0.f);
