...
#include "fpm/fixed.hpp"  // For btScalar
...
///The btScalar type abstracts floating point numbers, to easily switch between double and single floating point precision.
...
#else
	typedef fpm::fixed_40_24 btScalar;
	...
	//#if defined BT_USE_SSE_IN_API && defined (BT_USE_SSE)
			...
			#define BT_NAN (*(btScalar *)&btNanMask)
		#endif

		#ifndef BT_INFINITY
			static int btInfinityMask = 0x7F800000;
			#define BT_INFINITY (*(btScalar *)&btInfinityMask)
...
#define BT_DECLARE_ALIGNED_ALLOCATOR()                                                                     \
	...
	SIMD_FORCE_INLINE btScalar btFabs(btScalar x) { return abs(x); }
	SIMD_FORCE_INLINE btScalar btCos(btScalar x) { return cos(x); }
	SIMD_FORCE_INLINE btScalar btSin(btScalar x) { return sin(x); }
	SIMD_FORCE_INLINE btScalar btTan(btScalar x) { return tan(x); }
	SIMD_FORCE_INLINE btScalar btAcos(btScalar x)
...
#define btRecip(x) (btScalar(1.0) / btScalar(x))
	...
	//#define SIMD_EPSILON btScalar(1e-5)
	#define SIMD_EPSILON btScalar(111111e-12)
	//#define SIMD_INFINITY FLT_MAX 
	//#define SIMD_INFINITY btScalar(1e4)
	#define SIMD_INFINITY btScalar(127)
	#define BT_ONE 1.0f
	#define BT_ZERO 0.0f
	#define BT_TWO 2.0f
	#define BT_HALF 0.5f
...
SIMD_FORCE_INLINE btScalar btAtan2Fast(btScalar y, btScalar x)
{
	...
	btScalar coeff_2 = (btScalar)3.0f * (btScalar)coeff_1;
	...
	return (y < (btScalar)0.0f) ? -angle : angle;
}
...
SIMD_FORCE_INLINE btScalar btSelect(unsigned condition, btScalar valueIfConditionNonZero, btScalar valueIfConditionZero)
{
#ifdef BT_HAVE_NATIVE_FSEL
	return (btScalar)btFsel((btScalar)condition - btScalar(1.0f), valueIfConditionNonZero, valueIfConditionZero);
#else
...
////btSwapFloat/btSwapDouble will NOT return a btScalar, because the machine might 'correct' invalid floating point values
///Not all values of sign/exponent/mantissa are valid floating point numbers according to IEEE 754.
///When a floating point unit is faced with an invalid value, it may actually change the value, or worse, throw an exception.
///In most systems, running user mode code, you wouldn't get an exception, but instead the hardware/os/runtime will 'fix' the number for you.
///so instead of returning a btScalar/double, we return integer/long long integer
SIMD_FORCE_INLINE unsigned int btSwapEndianFloat(btScalar d)
{
	unsigned int a = 0;
...
SIMD_FORCE_INLINE btScalar btUnswapEndianFloat(unsigned int a)
{
	btScalar d = (btScalar)0.0f;
