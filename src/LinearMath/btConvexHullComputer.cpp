...
public:
	...
	public:
		...
		btScalar toScalar() const
		{
			return sign * ((m_denominator == 0) ? (btScalar)SIMD_INFINITY : (btScalar)m_numerator / m_denominator);
		}
	};
...
#ifdef DEBUG_CONVEX_HULL
				printf("      Angle is %f (%d) for ", (btScalar)btAtan(cot.toScalar()), (int)cot.isNaN());
				e->print();
#endif
				if (cot.isNaN())
				{
...
btScalar btConvexHullInternal::shrink(btScalar amount, btScalar clampAmount)
{
	...
	if (clampAmount > (btScalar)0)
	{
		btScalar minDist = (btScalar)SIMD_INFINITY;
...
bool btConvexHullInternal::shiftFace(Face* face, btScalar amount, btAlignedObjectArray<Vertex*> stack)
{
	...
	if (scaling[0] != (btScalar)0)
	{
		origShift[0] /= scaling[0];
	}
	if (scaling[1] != (btScalar)0)
	{
		origShift[1] /= scaling[1];
	}
	if (scaling[2] != (btScalar)0)
	{
	...
	if (cmp >= 0)
	{
		...
		do
		{
			...
			e->print();
			printf(", dot is %f (%f %lld)\n", (btScalar)dot.toScalar(), (btScalar)optDot.toScalar(), shiftedDot);
#endif
...
btScalar btConvexHullComputer::compute(const void* coords, bool doubleCoords, int stride, int count, btScalar shrink, btScalar shrinkClamp)
{
	...
	btScalar shift = (btScalar)0;
	if ((shrink > (btScalar)0) && ((shift = hull.shrink(shrink, shrinkClamp)) < (btScalar)0))
	{
		vertices.clear();
		edges.clear();
		faces.clear();
