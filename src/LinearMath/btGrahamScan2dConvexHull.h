...
inline void GrahamScanConvexHull2D(btAlignedObjectArray<GrahamVector3>& originalPoints, btAlignedObjectArray<GrahamVector3>& hull, const btVector3& normalAxis)
{
	...
	originalPoints[0].m_angle = (btScalar)-1e30f;
	for (int i = 1; i < originalPoints.size(); i++)
	{
		btVector3 ar = originalPoints[i] - originalPoints[0];
		btScalar ar1 = axis1.dot(ar);
		btScalar ar0 = axis0.dot(ar);
		if (ar1 * ar1 + ar0 * ar0 < (btScalar)FLT_EPSILON)
		{
			originalPoints[i].m_angle = (btScalar)0.0f;
		}
		else
		{
	...
	for (; i != originalPoints.size(); i++)
	{
		...
		while (!isConvex && hull.size() > 1)
		{
			...
			isConvex = btCross(a - b, a - originalPoints[i]).dot(normalAxis) > (btScalar)0;
			if (!isConvex)
				hull.pop_back();
			else
				hull.push_back(originalPoints[i]);
