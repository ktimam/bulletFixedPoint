...
public:
   ...
   * @param z Z value
   */
	SIMD_FORCE_INLINE btVector3(const int& _x, const int& _y, const int& _z)
	{
		m_floats[0] = (btScalar)_x;
		m_floats[1] = (btScalar)_y;
		m_floats[2] = (btScalar)_z;
 ...
 * @param The vector to add to this one */
   ...
   * @param s Scale factor */
	...
	{
#if defined(BT_USE_SSE_IN_API) && defined(BT_USE_SSE)
		float num = (static_cast<float>(s));
		__m128 vs = _mm_load_ss(&num);  //	(S 0 0 0)
...
// this code is not faster !
   ...
   * @param v The other vector in the dot product */
	SIMD_FORCE_INLINE btScalar dot(const btVector3& v) const
	{
		...
		vd = _mm_add_ss(vd, y);
		vd = _mm_add_ss(vd, z);
		return (btScalar)_mm_cvtss_f32(vd);
#elif defined(BT_USE_NEON)
   ...
   * @param v The other vector */
	...
	SIMD_FORCE_INLINE btScalar triple(const btVector3& v1, const btVector3& v2) const
	{
		...
		V = _mm_add_ss(V, y);
		V = _mm_add_ss(V, z);
		return (btScalar)_mm_cvtss_f32(V);
   ...
   * Note return values are 0,1,2 for x, y, or z */
	...
	{
#if defined(BT_USE_SSE_IN_API) && defined(BT_USE_SSE)
		float num = (static_cast<float>(rt));
		__m128 vrt = _mm_load_ss(&num);  //	(rt 0 0 0)
		btScalar s = btScalar(1.0) - rt;
		float nums = (static_cast<float>(s));
		__m128 vs = _mm_load_ss(&nums);  //	(S 0 0 0)
		vs = bt_pshufd_ps(vs, 0x80);  //	(S S S 0.0)
   ...
   * @param t The ration of this to v (t = 0 => return this, t=1 => return other) */
	...
	{
#if defined(BT_USE_SSE_IN_API) && defined(BT_USE_SSE)
		float num = (static_cast<float>(t));
		__m128 vt = _mm_load_ss(&num);  //	(t 0 0 0)
   ...
   */
	...
	SIMD_FORCE_INLINE void setValue(const int& _x, const int& _y, const int& _z)
	{
		m_floats[0] = (btScalar)_x;
		m_floats[1] = (btScalar)_y;
		m_floats[2] = (btScalar)_z;
	...
	void getSkewSymmetricMatrix(btVector3 * v0, btVector3 * v1, btVector3 * v2) const
	{
		...
		v2->mVec128 = V2;
#else
		v0->setValue((btScalar)0., -z(), y());
		v1->setValue(z(), (btScalar)0., -x());
		v2->setValue(-y(), x(), (btScalar)0.);
#endif
	}
...
SIMD_FORCE_INLINE btVector3 btVector3::rotate(const btVector3& wAxis, const btScalar _angle) const
{
	...
	float numsin = (static_cast<float>(ssin));
	__m128 vsin = _mm_load_ss(&numsin);  //	(S 0 0 0)
	float numcos = (static_cast<float>(scos));
	__m128 vcos = _mm_load_ss(&numcos);  //	(S 0 0 0)
...
SIMD_FORCE_INLINE long btVector3::maxDot(const btVector3* array, long array_count, btScalar& dotOut) const
{
	...
	long _maxdot_large(const btScalar* array, const btScalar* vec, unsigned long array_count, btScalar* dotOut);
#elif defined BT_USE_NEON
	const long scalar_cutoff = 4;
	extern long (*_maxdot_large)(const btScalar* array, const btScalar* vec, unsigned long array_count, btScalar* dotOut);
#endif
	if (array_count < scalar_cutoff)
#endif
	{
		btScalar maxDot1 = -(btScalar)SIMD_INFINITY;
		int i = 0;
		int ptIndex = -1;
...
#if (defined BT_USE_SSE && defined BT_USE_SIMD_VECTOR3 && defined BT_USE_SSE_IN_API) || defined(BT_USE_NEON)
	return _maxdot_large((btScalar*)array, (btScalar*)&m_floats[0], array_count, &dotOut);
#endif
}
...
#if defined BT_USE_SSE
	const long scalar_cutoff = 10;
	long _mindot_large(const btScalar* array, const btScalar* vec, unsigned long array_count, btScalar* dotOut);
#elif defined BT_USE_NEON
	const long scalar_cutoff = 4;
	extern long (*_mindot_large)(const btScalar* array, const btScalar* vec, unsigned long array_count, btScalar* dotOut);
#else
...
#endif
	{
		btScalar minDot = (btScalar)SIMD_INFINITY;
		int i = 0;
		int ptIndex = -1;
...
#if (defined BT_USE_SSE && defined BT_USE_SIMD_VECTOR3 && defined BT_USE_SSE_IN_API) || defined(BT_USE_NEON)
	return _mindot_large((btScalar*)array, (btScalar*)&m_floats[0], array_count, &dotOut);
...
SIMD_FORCE_INLINE void btVector3::serializeFloat(struct btVector3FloatData& dataOut) const
{
	...
	for (int i = 0; i < 4; i++)
		dataOut.m_floats[i] = btScalar(m_floats[i]);
}
