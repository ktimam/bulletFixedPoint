...
inline btScalar Profile_Get_Tick_Rate(void)
{
	//	return 1000000.f;
	return 1000.f;
}
...
bool CProfileNode::Return(void)
{
	if (--RecursionCounter == 0 && TotalCalls != 0)
	{
		...
		time -= StartTime;
		TotalTime += (btScalar)time / Profile_Get_Tick_Rate();
	}
 ...
 *=============================================================================================*/
btScalar CProfileManager::Get_Time_Since_Reset(void)
{
	unsigned long int time;
	Profile_Get_Ticks(&time);
	time -= ResetTime;
	return (btScalar)time / Profile_Get_Tick_Rate();
}
...
void CProfileManager::dumpRecursive(CProfileIterator* profileIterator, int spacing)
{
	...
	btScalar accumulated_time = 0, parent_time = profileIterator->Is_Root() ? CProfileManager::Get_Time_Since_Reset() : profileIterator->Get_Current_Parent_Total_Time();
	int i;
	...
	{
		numChildren++;
		btScalar current_total_time = profileIterator->Get_Current_Total_Time();
		accumulated_time += current_total_time;
		btScalar fraction = parent_time > SIMD_EPSILON ? (current_total_time / parent_time) * 100 : 0.f;
		{
			int i;
			for (i = 0; i < spacing; i++) printf(".");
		}
