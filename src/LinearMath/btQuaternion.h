...
public:
   ...
   * @param q The other quaternion */
	...
	* @param q The other quaternion */
	...
	{
		btScalar s_squared = (btScalar)1.f - m_floats[3] * m_floats[3];

		if (s_squared < btScalar(10.) * (btScalar)SIMD_EPSILON)  //Check for divide by zero
			return btVector3((btScalar)1.0, (btScalar)0.0, (btScalar)0.0);           // Arbitrary
		btScalar s = (btScalar)1.f / btSqrt(s_squared);
...
#else
	return btQuaternion((btScalar)0
		+w.x() * q.w() + w.y() * q.z() - w.z() * q.y(), (btScalar)0
		+w.y() * q.w() + w.z() * q.x() - w.x() * q.z(), (btScalar)0
		+w.z() * q.w() + w.x() * q.y() - w.y() * q.x(), (btScalar)
...
shortestArcQuat(const btVector3& v0, const btVector3& v1)  // Game Programming Gems 2.10. make sure v0,v1 are normalized
{
	...
	if (d < (btScalar)-1.0 + (btScalar)SIMD_EPSILON)
	{
		btVector3 n, unused;
		btPlaneSpace1(v0, n, unused);
		return btQuaternion(n.x(), n.y(), n.z(), (btScalar)0.0f);  // just pick any vector that is orthogonal to v0
	}

	btScalar s = btSqrt(((btScalar)1.0f + d) * (btScalar)2.0f);
	btScalar rs = (btScalar)1.0f / (btScalar)s;

	return btQuaternion(c.getX() * rs, c.getY() * rs, c.getZ() * rs, s * (btScalar)0.5f);
}
