...
inline long find_highest_bit(int256_t value) noexcept
{
    //assert(false);
    assert(value != 0);

    return boost::multiprecision::msb(value);
//#if defined(_MSC_VER)
//    unsigned long index;
//#if defined(_WIN64)
...
template <typename B, typename I, unsigned int F, typename T, typename boost::enable_if<boost::is_base_of<int128_t, T>>::type* = nullptr>
fixed<B, I, F> pow(fixed<B, I, F> base, T exp) noexcept
{
    using Fixed = fixed<B, I, F>;
...
fixed<B, I, F> exp2(fixed<B, I, F> x) noexcept
{
    ...
    return Fixed(1 << (int)x_int) * (((((fA * x + fB) * x + fC) * x + fD) * x + fE) * x + fF);
}
...
fixed<B, I, F> sqrt(fixed<B, I, F> x) noexcept
{
    ...
    for (I bit = I{1} << ((detail::find_highest_bit((unsigned long long)x.raw_value()) + F) / 2 * 2); bit != 0; bit >>= 2)
    {
        const I val = res + bit;
        res >>= 1;
        if (num >= val)
