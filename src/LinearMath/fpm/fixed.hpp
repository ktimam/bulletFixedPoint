...
public:
    ...
    inline fixed(const fixed& fxd) noexcept
    {
        m_value = fxd.m_value;
    }
    ...
    /*template <typename T, typename std::enable_if<boost::type_traits::is_base_of<int128_t, T>>::type* = nullptr>
    constexpr inline explicit fixed(T val) noexcept
        : m_value(static_cast<BaseType>(val* FRACTION_MULT))
    {}*/
    template <typename T, typename boost::enable_if<boost::is_base_of<int128_t, T>>::type* = nullptr>
    constexpr inline explicit fixed(T val) noexcept
        : m_value(static_cast<BaseType>(val* FRACTION_MULT))
    {  }
    ...
    // Explicit conversion to a int128_t
    template <typename T, typename boost::enable_if<boost::is_base_of<int128_t, T>>::type* = nullptr>
    constexpr inline explicit operator T() const noexcept
    {
        return static_cast<T>(m_value / FRACTION_MULT);
    ...
    {
        m_value += (int128_t)(y * FRACTION_MULT);
        return *this;
    }
...
using fixed_40_24 = fixed<int128_t, int256_t, 32>;
//
// Addition
//
