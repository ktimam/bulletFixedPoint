...
#ifdef __APPLE__
#include <stdint.h>
typedef btScalar float4 __attribute__((vector_size(16)));
#else
...
long _maxdot_large(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
long _maxdot_large(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult)
{
	...
	btScalar btinfinityscalar = BT_INFINITY;
	float btinfinityf = -static_cast<float>(btinfinityscalar);
	float4 dotMax = btAssign128(btinfinityf, btinfinityf, btinfinityf, btinfinityf);
	float num = (static_cast<float>(*vec));
	float4 vvec = _mm_loadu_ps(&num);
	...
	if (localCount)
	{
		...
		size_t byteIndex = -(localCount) * sizeof(btScalar);
	...
	float num2 = (static_cast<float>(*dotResult));
	_mm_store_ss(&num2, dotMax);
	return maxIndex;
}

long _mindot_large(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
...
long _mindot_large(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult)
{
	...
	btScalar btinfinityscalar = BT_INFINITY;
	float btinfinityf = -static_cast<float>(btinfinityscalar);
	float4 dotmin = btAssign128(btinfinityf, btinfinityf, btinfinityf, btinfinityf);
	float num = (static_cast<float>(*vec));
	float4 vvec = _mm_loadu_ps(&num);
	float4 vHi = btCastiTo128f(_mm_shuffle_epi32(btCastfTo128i(vvec), 0xaa));  /// zzzz
	float4 vLo = _mm_movelh_ps(vvec, vvec);                                    /// xyxy
...
static long _maxdot_large_v0(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
static long _maxdot_large_v1(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
static long _maxdot_large_sel(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
static long _mindot_large_v0(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
static long _mindot_large_v1(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);
static long _mindot_large_sel(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult);

long (*_maxdot_large)(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult) = _maxdot_large_sel;
long (*_mindot_large)(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult) = _mindot_large_sel;
...
#else
//support 64bit arm
#define vld1q_f32_aligned_postincrement(_ptr) ({ float32x4_t _r = ((float32x4_t*)(_ptr))[0]; (_ptr) = (const btScalar*) ((const char*)(_ptr) + 16L); /*return*/ _r; })
#endif

long _maxdot_large_v0(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult)
{
	unsigned long i = 0;
...
long _maxdot_large_v1(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult)
{
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
...
long _mindot_large_v0(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult)
{
	unsigned long i = 0;
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	float32x2_t vLo = vget_low_f32(vvec);
...
long _mindot_large_v1(const btScalar *vv, const btScalar *vec, unsigned long count, btScalar *dotResult)
{
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	float32x4_t vLo = vcombine_f32(vget_low_f32(vvec), vget_low_f32(vvec));
	float32x4_t vHi = vdupq_lane_f32(vget_high_f32(vvec), 0);
