...
static SIMD_FORCE_INLINE btScalar _root3(btScalar x)
{
	...
	r -= (btScalar)1. / (btScalar)3. * (r - x / (r * r));
	r -= (btScalar)1. / (btScalar)3. * (r - x / (r * r));
	r -= (btScalar)1. / (btScalar)3. * (r - x / (r * r));
	r -= (btScalar)1. / (btScalar)3. * (r - x / (r * r));
	r -= (btScalar)1. / (btScalar)3. * (r - x / (r * r));
...
{  // solve cubic equation x^3 + a*x^2 + b*x + c = 0
	...
	{  //<<-- FIXED!
		...
		q = -2 * sqrt(q);
		x[0] = q * cos(t / 3) - a;
		x[1] = q * cos((t + (btScalar)TwoPi) / (btScalar)3) - a;
		x[2] = q * cos((t - (btScalar)TwoPi) / (btScalar)3) - a;
		return (3);
	}
	else
	{
		...
		if (r < (btScalar)0)
			A = -A;
		B = (A == (btScalar)0 ? (btScalar)0 : q / A);

		a /= 3;
		x[0] = (A + B) - a;
		x[1] = (btScalar)-0.5 * (A + B) - a;
		x[2] = (btScalar)0.5 * sqrt((btScalar)3.) * (A - B);
		if (btFabs(x[2]) < eps)
		{
			x[2] = x[1];
			return (2);
...
int SolveP4Bi(btScalar* x, btScalar b, btScalar d)  // solve equation x^4 + b*x^2 + d = 0
{
	...
	if (D >= (btScalar)0)
	{
		...
		if (x2 >= (btScalar)0)                  // 0 <= x2 <= x1, 4 real roots
		{
		...
		if (x1 < (btScalar)0)  // x2 <= x1 < 0, two pair of imaginary roots
		{
			btScalar sx1 = sqrt(-x1);
			btScalar sx2 = sqrt(-x2);
			x[0] = (btScalar)0;
	...
	{  // if( D < 0 ), two pair of compex roots
		btScalar sD2 = (btScalar)0.5 * sqrt(-D);
...
int SolveP4De(btScalar* x, btScalar b, btScalar c, btScalar d)  // solve equation x^4 + b*x^2 + c*x + d
{
	...
	if (btFabs(c) < (btScalar)1e-14 * (btFabs(b) + btFabs(d)))
	...
	if (res3 > 1)  // 3 real roots,
	{
		...
		if (x[0] > (btScalar)0)  // all roots are positive
		{
...
{  // solve equation x^4 + a*x^3 + b*x^2 + c*x + d by Dekart-Euler method
	// move to a=0:
	btScalar d1 = d + (btScalar)0.25 * a * ((btScalar)0.25 * b * a - (btScalar)3. / (btScalar)64 * a * a * a - c);
