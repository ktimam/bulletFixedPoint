...
public:
	...
	{
		int _coords[] = {
			static_cast<int>(m_point[0] * (btScalar)1000.0f + (btScalar)1.0f),
			static_cast<int>(m_point[1] * (btScalar)1333.0f),
			static_cast<int>(m_point[2] * (btScalar)2133.0f + (btScalar)3.0f)};
	...
	SIMD_FORCE_INLINE void interpolate_normals(btVector3 *normals, int normal_count)
	{
		...
		if (vec_sum_len < (btScalar)CONTACT_DIFF_EPSILON) return;
