	...
	const btVector3& bvhQuantization)
{
	...
	out[0] = static_cast<unsigned short>(v.getX() + (btScalar)0.5f);
	out[1] = static_cast<unsigned short>(v.getY() + (btScalar)0.5f);
	out[2] = static_cast<unsigned short>(v.getZ() + (btScalar)0.5f);
}
