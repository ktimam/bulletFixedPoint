	...
	const btContactArray& contacts, bool normal_contact_average)
{
	...
	for (i = 1; i < keycontacts.size(); i++)
	{
		...
		{
			//merge contact
			if (pcontact->m_depth - (btScalar)CONTACT_DIFF_EPSILON > scontact->m_depth)  //)
			{
				*pcontact = *scontact;
				coincident_count = 0;
			}
			...
			{
				if (btFabs(pcontact->m_depth - scontact->m_depth) < (btScalar)CONTACT_DIFF_EPSILON)
				{
...
void btContactArray::merge_contacts_unique(const btContactArray& contacts)
{
	...
	//divide
	btScalar divide_average = (btScalar)1.0f / ((btScalar)contacts.size());
