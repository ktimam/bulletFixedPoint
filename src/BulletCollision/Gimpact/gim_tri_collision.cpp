...
public:
		...
		GUINT &edge_index1)
	{
		...
		else if (D1 * D2 > (btScalar)0.0f || D0 != (btScalar)0.0f)
		{
	...
	*/
	SIMD_FORCE_INLINE GUINT cross_line_intersection_test()
	{
		...
		if (midpoint_u < midpoint_v)
		{
			...
			if (isect_u[1] + (btScalar)MIN_EDGE_EDGE_DIS < isect_v[0])  //calc distance between two lines instead
			{
				SEGMENT_COLLISION(
				...
				edge_edge_dir *= (btScalar)1.0f / distances[2];  // normalize
			}
			else
			{
		...
		else
		{
			...
			if (isect_v[1] + (btScalar)MIN_EDGE_EDGE_DIS < isect_u[0])  //calc distance between two lines instead
			{
				SEGMENT_COLLISION(
		...
		GIM_TRIANGLE_CONTACT_DATA &contacts)
	{
		...
		if (du0du1 > (btScalar)0.0f && du0du2 > (btScalar)0.0f)  // same sign on all of them + not equal 0 ?
		{
			if (du[0] < (btScalar)0)  //we need test behind the triangle plane
			{
				...
				VEC_SCALE_4(tv_plane, (btScalar)-1.0f, tv_plane);
			}
			else
			{
		...
		else
		{
			...
			distances[0] = (du[0] + du[1] + du[2]) / (btScalar)3.0f;  //centroid
		...
		if (dv0dv1 > (btScalar)0.0f && dv0dv2 > (btScalar)0.0f)  // same sign on all of them + not equal 0 ?
		{
			if (dv[0] < (btScalar)0)  //we need test behind the triangle plane
			{
				...
				VEC_SCALE_4(tu_plane, (btScalar)-1.0f, tu_plane);
			}
			else
			{
		...
		{
			//Look if we need to invert the triangle
			distances[1] = (dv[0] + dv[1] + dv[2]) / (btScalar)3.0f;  //centroid

			if (distances[1] < (btScalar)0.0f)
			{
				//reorder triangle v
				VEC_SWAP(tu_vertices[0], tu_vertices[1]);
				VEC_SCALE_4(tu_plane, (btScalar)-1.0f, tu_plane);
		...
		else  //clip V points against U
		{
			...
			contacts.m_separating_normal *= (btScalar)-1.f;
		}
