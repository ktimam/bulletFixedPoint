	...
	const CLASS_POINT& point, const CLASS_PLANE* planes, GUINT plane_count)
{
	...
	for (GUINT _i = 0; _i < plane_count; ++_i)
	{
		...
		if (_dis > (btScalar)0.0f) return false;
	}
	return true;
}
	...
	CLASS_POINT& pout, T& tparam)
{
	...
	if (_dotdir < (btScalar)PLANEDIREPSILON)
	{
		return false;
	}
	...
	T tmin, T tmax)
{
	GREAL _dis, _dotdir;
	_dotdir = VEC_DOT(plane, vDir);
	if (btFabs(_dotdir) < (btScalar)PLANEDIREPSILON)
	{
		tparam = tmax;
		return 0;
	}
	_dis = DISTANCE_PLANE_POINT(plane, vPoint);
	char returnvalue = _dis < (btScalar)0.0f ? 2 : 1;
	tparam = -_dis / _dotdir;
	...
	const CLASS_POINT& e1, const CLASS_POINT& e2)
{
	...
	if (_scalar < (btScalar)0.0f)
	{
		VEC_COPY(cp, e1);
	}
	else if (_scalar > (btScalar)1.0f)
	{
		VEC_COPY(cp, e2);
	}
	else
	...
	CLASS_POINT& vPointB)
{
	...
	if (_tp < (btScalar)G_EPSILON)  //ARE PARALELE
	{
		...
		//mid points
		n[0] = (_M[0] + _M[1]) * (btScalar)0.5f;
		n[1] = (_M[2] + _M[3]) * (btScalar)0.5f;
