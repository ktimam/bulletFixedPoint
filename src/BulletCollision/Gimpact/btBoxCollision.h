...
public:
	...
	{
		m_min[0] = (btScalar)SIMD_INFINITY;
		m_min[1] = (btScalar)SIMD_INFINITY;
		m_min[2] = (btScalar)SIMD_INFINITY;
		m_min[3] = (btScalar)0.f;
		m_max[0] = (btScalar)-SIMD_INFINITY;
		m_max[1] = (btScalar)-SIMD_INFINITY;
		m_max[2] = (btScalar)-SIMD_INFINITY;
		m_max[3] = (btScalar)0.f;
	}
	...
	*/
	SIMD_FORCE_INLINE bool collide_ray(const btVector3 &vorigin, const btVector3 &vdir) const
	{
		...
		if (BT_GREATER(Dx, extents[0]) && Dx * vdir[0] >= (btScalar)0.0f) return false;
		btScalar Dy = vorigin[1] - center[1];
		if (BT_GREATER(Dy, extents[1]) && Dy * vdir[1] >= (btScalar)0.0f) return false;
		btScalar Dz = vorigin[2] - center[2];
		if (BT_GREATER(Dz, extents[2]) && Dz * vdir[2] >= (btScalar)0.0f) return false;
	...
	SIMD_FORCE_INLINE eBT_PLANE_INTERSECTION_TYPE plane_classify(const btVector4 &plane) const
	{
		...
		if (plane[3] > _fmax + (btScalar)BOX_PLANE_EPSILON)
		{
			return BT_CONST_BACK_PLANE;  // 0
		}

		if (plane[3] + (btScalar)BOX_PLANE_EPSILON >= _fmin)
		{
