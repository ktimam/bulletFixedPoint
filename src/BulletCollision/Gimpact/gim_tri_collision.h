...
if 0.0<= u+v <=1.0 then they are inside of triangle
	...
	*/
		...
		GREAL &u, GREAL &v) const
	{
		...
		else
		{
			...
			else if (sumuv - (btScalar)1.0f > (btScalar)G_EPSILON)
			{
				return false;
			}
		}
	...
	*/
	SIMD_FORCE_INLINE bool is_point_inside(const btVector3 &point, const btVector3 &tri_normal) const
	{
		...
		if (dist - m_margin > (btScalar)0.0f) return false;  // outside plane
	...
	SIMD_FORCE_INLINE bool ray_collision(
		...
		GREAL &tparam, GREAL tmax = (btScalar)G_REAL_INFINITY)
	{
		btVector4 faceplane;
		{
