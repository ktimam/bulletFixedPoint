...
struct btDbvtAabbMm
{
	...
	DBVT_INLINE btVector3 Center() const { return ((mi + mx) / (btScalar)2); }
	DBVT_INLINE btVector3 Lengths() const { return (mx - mi); }
	DBVT_INLINE btVector3 Extents() const { return ((mx - mi) / (btScalar)2); }
...
struct btDbvntNode
{
	...
	btDbvntNode(const btDbvtNode* n)
		: volume(n->volume)
		, normal((btScalar)0, (btScalar)0, (btScalar)0)
		, angle(0)
		, data(n->data)
	{
		childs[0] = 0;
	...
	DBVT_CHECKTYPE
		if (root)
		{
			...
			do
			{
				...
				btScalar tmin = (btScalar)1.f, lambda_min = (btScalar)0.f;
	...
	DBVT_IPOLICY)
{
	DBVT_CHECKTYPE
		if (root)
		{
			...
			{
				signs[i] = ((normals[i].x() >= (btScalar)0) ? 1 : 0) +
					((normals[i].y() >= (btScalar)0) ? 2 : 0) +
					((normals[i].z() >= (btScalar)0) ? 4 : 0);
			}
			stack.reserve(SIMPLE_STACKSIZE);
	...
	bool fsort)
{
	DBVT_CHECKTYPE
		if (root)
		{
			const unsigned srtsgns = (sortaxis[0] >= (btScalar)0 ? 1 : 0) +
				(sortaxis[1] >= (btScalar)0 ? 2 : 0) +
				(sortaxis[2] >= (btScalar)0 ? 4 : 0);
			const int inside = (1 << count) - 1;
			btAlignedObjectArray<sStkNPS> stock;
