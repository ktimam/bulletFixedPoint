...
#endif
	{
		...
		{ /* dynamic set				*/
			...
			{ /* Moving				*/
				...
				btVector3 velocity(((proxy->m_aabbMax - proxy->m_aabbMin) / (btScalar)2) * m_prediction);
				if (delta[0] < (btScalar)0) velocity[0] = -velocity[0];
				if (delta[1] < (btScalar)0) velocity[1] = -velocity[1];
				if (delta[2] < (btScalar)0) velocity[2] = -velocity[2];
				if (
...
void btDbvtBroadphase::getBroadphaseAabb(btVector3& aabbMin, btVector3& aabbMax) const
{
	...
	if (!m_sets[0].empty())
		...
		bounds = btDbvtVolume::FromCR(btVector3((btScalar)0, (btScalar)0, (btScalar)0), (btScalar)0);
	aabbMin = bounds.Mins();
	aabbMax = bounds.Maxs();
}
