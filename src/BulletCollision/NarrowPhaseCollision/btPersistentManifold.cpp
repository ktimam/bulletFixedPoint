...
btPersistentManifold::btPersistentManifold(const btPersistentManifold&)
	: btTypedObject(BT_PERSISTENT_MANIFOLD_TYPE),
	m_body0(0),
	m_body1(0),
	m_cachedPoints(0),
...
const char* btPersistentManifold::serialize(const class btPersistentManifold* manifold, void* dataBuffer, class btSerializer* serializer) const
{
	...
	dataOut->m_contactBreakingThreshold = static_cast<float>(manifold->getContactBreakingThreshold());
	dataOut->m_contactProcessingThreshold = static_cast<float>(manifold->getContactProcessingThreshold());
	...
	for (int i = 0; i < this->getNumContacts(); i++)
	{
		...
		dataOut->m_pointCacheAppliedImpulse[i] = static_cast<float>(pt.m_appliedImpulse);
		dataOut->m_pointCachePrevRHS[i] = static_cast<float>(pt.m_prevRHS);
		dataOut->m_pointCacheAppliedImpulseLateral1[i] = static_cast<float>(pt.m_appliedImpulseLateral1);
		dataOut->m_pointCacheAppliedImpulseLateral2[i] = static_cast<float>(pt.m_appliedImpulseLateral2);
		...
		dataOut->m_pointCacheDistance[i] = static_cast<float>(pt.m_distance1);
		dataOut->m_pointCacheCombinedContactDamping1[i] = static_cast<float>(pt.m_combinedContactDamping1);
		dataOut->m_pointCacheCombinedContactStiffness1[i] = static_cast<float>(pt.m_combinedContactStiffness1);
		dataOut->m_pointCacheLifeTime[i] = pt.m_lifeTime;
		dataOut->m_pointCacheFrictionCFM[i] = static_cast<float>(pt.m_frictionCFM);
		dataOut->m_pointCacheContactERP[i] = static_cast<float>(pt.m_contactERP);
		dataOut->m_pointCacheContactCFM[i] = static_cast<float>(pt.m_contactCFM);
		...
		dataOut->m_pointCacheCombinedFriction[i] = static_cast<float>(pt.m_combinedFriction);
		...
		dataOut->m_pointCacheCombinedRollingFriction[i] = static_cast<float>(pt.m_combinedRollingFriction);
		dataOut->m_pointCacheCombinedSpinningFriction[i] = static_cast<float>(pt.m_combinedSpinningFriction);
		dataOut->m_pointCacheCombinedRestitution[i] = static_cast<float>(pt.m_combinedRestitution);
		dataOut->m_pointCacheContactMotion1[i] = static_cast<float>(pt.m_contactMotion1);
		dataOut->m_pointCacheContactMotion2[i] = static_cast<float>(pt.m_contactMotion2);
	}
