...
public:
	...
	btPersistentManifold(const btPersistentManifold&);
	btPersistentManifold& operator=(const btPersistentManifold& manifoldToCopy)
	{		
		return *this;
	}
	...
	void removeContactPoint(int index)
	{
		...
		if (index != lastUsedIndex)
		{
			...
			m_pointCache[lastUsedIndex].m_appliedImpulse = (btScalar)0.f;
			m_pointCache[lastUsedIndex].m_prevRHS = (btScalar)0.f;
			m_pointCache[lastUsedIndex].m_contactPointFlags = 0;
			m_pointCache[lastUsedIndex].m_appliedImpulseLateral1 = (btScalar)0.f;
			m_pointCache[lastUsedIndex].m_appliedImpulseLateral2 = (btScalar)0.f;
...
#define MAINTAIN_PERSISTENCY 1
		...
		if (newPoint.m_contactPointFlags & BT_CONTACT_FLAG_FRICTION_ANCHOR)
		{
			...
			btScalar eps = (btScalar)0;  //we could allow to enlarge or shrink the tolerance to check against the friction cone a bit, say 1e-7
