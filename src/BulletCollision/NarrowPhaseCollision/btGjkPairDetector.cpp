...
static void btComputeSupport(const btConvexShape *convexA, const btTransform &localTransA, const btConvexShape *convexB, const btTransform &localTransB, const btVector3 &dir, bool check2d, btVector3 &supAworld, btVector3 &supBworld, btVector3 &aMinb)
{
	...
	if (check2d)
	{
		supAworld[2] = (btScalar)0.f;
		supBworld[2] = (btScalar)0.f;
	}
...
static btVector3 ccd_vec3_origin((btScalar)0, (btScalar)0, (btScalar)0);
...
#endif
{
	m_cachedSeparatingDistance = (btScalar)0.f;
	...
	m_cachedSeparatingAxis.setValue((btScalar)0, (btScalar)1, (btScalar)0);
	...
	m_lastUsedMethod = -1;
	int status = -2;
	btVector3 orgNormalInB((btScalar)0, (btScalar)0, (btScalar)0);
	...
	//note, for large differences in shapes, use double precision build!
	{
		...
		btVector3 dir((btScalar)1, (btScalar)0, (btScalar)0);
		...
		if (1)
		{
			...
			//while (true)
			{
				...
				if (check2d)
				{
					pWorld[2] = (btScalar)0.f;
					qWorld[2] = (btScalar)0.f;
				}
				...
				if ((delta > btScalar(0.0)) && ((float)delta * (float)delta > squaredDistance * (float)input.m_maximumDistanceSquared))
				{
				...
				float f0 = squaredDistance - (float)delta;
				...
				if ((float)newCachedSeparatingAxis.length2() < REL_ERROR2)
				{
				...
				squaredDistance = (float)newCachedSeparatingAxis.length2();
#if 0
				...
				//are we getting any closer ?
				if (previousSquaredDistance - squaredDistance <= (float)SIMD_EPSILON * previousSquaredDistance)
				{
			...
			if (checkSimplex)
			{
				...
				float lenSqr = (float)m_cachedSeparatingAxis.length2();
				...
				if (lenSqr > (float)SIMD_EPSILON * (float)SIMD_EPSILON)
				{
					btScalar rlen = btScalar(1.) / btSqrt((btScalar)lenSqr);
					normalInB *= rlen;  //normalize

					btScalar s = btSqrt((btScalar)squaredDistance);
	...
	if (isValid && ((distance < (btScalar)0) || (distance * distance < input.m_maximumDistanceSquared)))
	{
