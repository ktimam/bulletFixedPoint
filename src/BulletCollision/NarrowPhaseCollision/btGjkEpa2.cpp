...
struct GJK
{
	...
	void Initialize()
	{
		m_ray = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
		m_nfree = 0;
	...
	eStatus::_ Evaluate(const tShape& shapearg, const btVector3& guess)
	{
		...
		appendvertice(m_simplices[0], sqrl > (btScalar)0 ? -m_ray : btVector3((btScalar)1, (btScalar)0, (btScalar)0));
		...
		do
		{
			...
			if (((rl - alpha) - (GJK_ACCURACY * rl)) <= (btScalar)0)
			{ /* Return old simplex				*/
				removevertice(m_simplices[m_current]);
				break;
			}
	...
	bool EncloseOrigin()
	{
		switch (m_simplex->rank)
		{
			case 1:
			{
				for (U i = 0; i < 3; ++i)
				{
					btVector3 axis = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
					axis[i] = (btScalar)1;
			...
			case 4:
			{
				if (btFabs(det(m_simplex->c[0]->w - m_simplex->c[3]->w,
							   ...
							   m_simplex->c[2]->w - m_simplex->c[3]->w)) > (btScalar)0)
					return (true);
			}
			break;
		}
...
struct EPA
{
	...
	{
		m_status = eStatus::Failed;
		m_normal = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
		m_depth = (btScalar)0;
		m_nextsv = 0;
	...
	eStatus::_ Evaluate(GJK& gjk, const btVector3& guess)
	{
		...
		if ((simplex.rank > 1) && gjk.EncloseOrigin())
		{
			...
			if (gjk.det(simplex.c[0]->w - simplex.c[3]->w,
						...
						simplex.c[2]->w - simplex.c[3]->w) < (btScalar)0)
			{
		...
		if (nl > (btScalar)0)
			...
			m_normal = btVector3((btScalar)1, (btScalar)0, (btScalar)0);
		m_depth = (btScalar)0;
		m_result.rank = 1;
		m_result.c[0] = simplex.c[0];
		m_result.p[0] = (btScalar)1;
		return (m_status);
	}
	bool getedgedist(sFace* face, sSV* a, sSV* b, btScalar& dist)
	{
		...
		if (a_dot_nab < (btScalar)0)
		{
			...
			if (a_dot_ba > (btScalar)0)
			{
				// Pick distance vertex a
				dist = a->w.length();
			}
			else if (b_dot_ba < (btScalar)0)
			{
					   ...
					   bool withmargins)
{
	/* Results		*/
	results.witnesses[0] =
		results.witnesses[1] = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
							   ...
							   sResults& results)
{
	...
	{
		btVector3 w0 = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
		btVector3 w1 = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
										 ...
										 sResults& results)
{
	...
	btTransform wtrs1(btQuaternion((btScalar)0, (btScalar)0, (btScalar)0, (btScalar)1), position);
	Initialize(shape0, wtrs0, &shape1, wtrs1, results, shape, false);
	GJK gjk;
	GJK::eStatus::_ gjk_status = gjk.Evaluate(shape, btVector3((btScalar)1, (btScalar)1, (btScalar)1));
	if (gjk_status == GJK::eStatus::Valid)
	{
		btVector3 w0 = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
		btVector3 w1 = btVector3((btScalar)0, (btScalar)0, (btScalar)0);
