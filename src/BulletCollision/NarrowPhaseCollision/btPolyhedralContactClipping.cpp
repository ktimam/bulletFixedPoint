	...
	for (ve = 0; ve < numVerts; ve++)
	{
		...
		if (ds < (btScalar)0)
		{
			...
			else
			{
				...
				ppVtxOut.push_back(firstVertex.lerp(endVertex, btScalar(ds * (btScalar)1.f / (ds - de))));
			}
		}
		else
		{
...
static bool TestSepAxis(const btConvexPolyhedron& hullA, const btConvexPolyhedron& hullB, const btTransform& transA, const btTransform& transB, const btVector3& sep_axis, btScalar& depth, btVector3& witnessPointA, btVector3& witnessPointB)
{
	...
	btAssert(d0 >= (btScalar)0.0f);
	btScalar d1 = Max1 - Min0;
	btAssert(d1 >= (btScalar)0.0f);
	if (d0 < d1)
	{
		depth = d0;
...
inline void BoxSupport(const btScalar extents[3], const btScalar sv[3], btScalar p[3])
{
	...
	p[0] = sv[0] < (btScalar)0.0f ? -extents[0] : extents[0];
	p[1] = sv[1] < (btScalar)0.0f ? -extents[1] : extents[1];
	p[2] = sv[2] < (btScalar)0.0f ? -extents[2] : extents[2];
}
	...
	const btVector3& dirB, btScalar hlenB)
{
	...
	btScalar denom = (btScalar)1.0f - dirA_dot_dirB * dirA_dot_dirB;

	if (denom == (btScalar)0.0f)
	{
		tA = (btScalar)0.0f;
	}
	else
...
bool btPolyhedralContactClipping::findSeparatingAxis(const btConvexPolyhedron& hullA, const btConvexPolyhedron& hullB, const btTransform& transA, const btTransform& transB, btVector3& sep, btDiscreteCollisionDetectorInterface::Result& resultOut)
{
	...
	btScalar dmin = (btScalar)FLT_MAX;
	int curPlaneTests = 0;
	...
	for (int i = 0; i < numFacesA; i++)
	{
		...
		if (DeltaC2.dot(faceANormalWS) < (btScalar)0)
			faceANormalWS *= (btScalar)-1.f;
	...
	for (int i = 0; i < numFacesB; i++)
	{
		...
		if (DeltaC2.dot(WorldNormal) < (btScalar)0)
			WorldNormal *= (btScalar)-1.f;
	...
	btVector3 witnessPointA((btScalar)0, (btScalar)0, (btScalar)0), witnessPointB((btScalar)0, (btScalar)0, (btScalar)0);
	...
	for (int e0 = 0; e0 < hullA.m_uniqueEdges.size(); e0++)
	{
		...
		for (int e1 = 0; e1 < hullB.m_uniqueEdges.size(); e1++)
		{
			...
			if (!IsAlmostZero(Cross))
			{
				...
				if (DeltaC2.dot(Cross) < (btScalar)0)
					Cross *= (btScalar)-1.f;
	...
	if (edgeA >= 0 && edgeB >= 0)
	{
		...
		btScalar hlenB = (btScalar)1e30f;
		btScalar hlenA = (btScalar)1e30f;
		...
		if (nlSqrt > (btScalar)SIMD_EPSILON)
		{
			...
			ptsVector *= (btScalar)1.f / nl;
			if (ptsVector.dot(DeltaC2) < (btScalar)0.f)
			{
				ptsVector *= (btScalar)-1.f;
			}
	...
	if ((DeltaC2.dot(sep)) < (btScalar)0.0f)
		sep = -sep;
...
void btPolyhedralContactClipping::clipHullAgainstHull(const btVector3& separatingNormal1, const btConvexPolyhedron& hullA, const btConvexPolyhedron& hullB, const btTransform& transA, const btTransform& transB, const btScalar minDist, btScalar maxDist, btVertexArray& worldVertsB1, btVertexArray& worldVertsB2, btDiscreteCollisionDetectorInterface::Result& resultOut)
{
	...
	int closestFaceB = -1;
	btScalar dmax = (btScalar)-FLT_MAX;
	{
