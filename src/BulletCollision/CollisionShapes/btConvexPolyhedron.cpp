...
{
	m_localCenter.setValue((btScalar)0, (btScalar)0, (btScalar)0);
	btScalar TotalArea = (btScalar)0.0f;
	for (int i = 0; i < m_faces.size(); i++)
	{
		int numVertices = m_faces[i].m_indices.size();
		...
		for (int j = 1; j <= NbTris; j++)
		{
			...
			btVector3 Center = (p0 + p1 + p2) / (btScalar)3.0f;
	...
	if (1)
	{
		...
		btScalar MinX = (btScalar)FLT_MAX;
		btScalar MinY = (btScalar)FLT_MAX;
		btScalar MinZ = (btScalar)FLT_MAX;
		btScalar MaxX = (btScalar)-FLT_MAX;
		btScalar MaxY = (btScalar)-FLT_MAX;
		btScalar MaxZ = (btScalar)-FLT_MAX;
		...
		const btScalar r = m_radius / sqrt((btScalar)3.0f);
		const int LargestExtent = mE.maxAxis();
		const btScalar Step = (mE[LargestExtent] * (btScalar)0.5f - r) / (btScalar)1024.0f;
		...
		{
			// Refine the box
			const btScalar Step = (m_radius - r) / (btScalar)1024.0f;
...
{
	minProj = (btScalar)FLT_MAX;
	maxProj = (btScalar)-FLT_MAX;
