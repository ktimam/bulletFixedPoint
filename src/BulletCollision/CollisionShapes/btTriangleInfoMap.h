...
struct btTriangleInfoMap : public btInternalTriangleInfoMap
{
	...
	btTriangleInfoMap()
	{
		m_convexEpsilon = (btScalar)0.00f;
		m_planarEpsilon = (btScalar)0.0001f;
...
SIMD_FORCE_INLINE const char* btTriangleInfoMap::serialize(void* dataBuffer, btSerializer* serializer) const
{
	...
	tmapData->m_convexEpsilon = (btScalar)m_convexEpsilon;
	tmapData->m_planarEpsilon = (btScalar)m_planarEpsilon;
	tmapData->m_equalVertexThreshold = (btScalar)m_equalVertexThreshold;
	tmapData->m_edgeDistanceThreshold = (btScalar)m_edgeDistanceThreshold;
	tmapData->m_zeroAreaThreshold = (btScalar)m_zeroAreaThreshold;
	...
	if (tmapData->m_valueArrayPtr)
	{
		...
		for (int i = 0; i < numElem; i++, memPtr++)
		{
			memPtr->m_edgeV0V1Angle = (btScalar)m_valueArray[i].m_edgeV0V1Angle;
			memPtr->m_edgeV1V2Angle = (btScalar)m_valueArray[i].m_edgeV1V2Angle;
			memPtr->m_edgeV2V0Angle = (btScalar)m_valueArray[i].m_edgeV2V0Angle;
