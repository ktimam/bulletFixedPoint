...
bool btMiniSDF::load(const char* data, int size)
{
	...
	for (unsigned int i = 0; i < n_nodes0; i++)
	{
		...
		ds.read(n_nodes1);
		btAlignedObjectArray<btScalar>& nodes = m_nodes[i];
		nodes.resize(n_nodes1);
		for (int j = 0; j < nodes.size(); j++)
		{
			btScalar& node = nodes[j];
			ds.read(node);
		}
	}
...
btMiniSDF::subdomain(btMultiIndex const& ijk) const
{
	btAssert(m_isValid);
	btVector3 tmp;
	tmp.m_floats[0] = m_cell_size[0] * ijk.ijk[0];
	tmp.m_floats[1] = m_cell_size[1] * ijk.ijk[1];
	tmp.m_floats[2] = m_cell_size[2] * ijk.ijk[2];
...
btMiniSDF::shape_function_(btVector3 const& xi, btShapeGradients* gradient) const
{
	...
	if (gradient)
	{
		...
		dN.topRowsDivide(8, (btScalar)64.0);
		...
		dN.bottomRowsMul(32u - 8u, (btScalar)9.0 / (btScalar)64.0);
	}

	return res;
}

bool btMiniSDF::interpolate(unsigned int field_id, btScalar& dist, btVector3 const& x,
							btVector3* gradient) const
{
	...
	btVector3 c0 = btVector3((btScalar)2.0, (btScalar)2.0, (btScalar)2.0) / denom;
	...
	if (!gradient)
	{
		...
		btScalar phi = (btScalar)0.0;
		btShapeMatrix N = shape_function_(xi, 0);
		for (unsigned int j = 0u; j < 32u; ++j)
		{
			unsigned int v = cell.m_cells[j];
			btScalar c = m_nodes[field_id][v];
			if (c == (btScalar)DBL_MAX)
			{
				return false;
				;
			}
