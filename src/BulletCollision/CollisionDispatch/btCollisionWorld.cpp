...
void btCollisionWorld::updateSingleAabb(btCollisionObject* colObj)
{
	...
	float dist2 = (float)(maxAabb - minAabb).length2();
	double threshold = 1e12;

	//moving objects should be moderately sized, probably something wrong if not
	if (colObj->isStaticObject() || (dist2 < threshold))
	{
...
void btCollisionWorld::convexSweepTest(const btConvexShape* castShape, const btTransform& convexFromWorld, const btTransform& convexToWorld, ConvexResultCallback& resultCallback, btScalar allowedCcdPenetration) const
{
	...
	/* Compute AABB that encompasses angular movement */
	{
		...
		zeroLinVel.setValue((btScalar)0, (btScalar)0, (btScalar)0);
		btTransform R;
		R.setIdentity();
...
public:
	...
	virtual void processTriangle(btVector3* triangle, int partId, int triangleIndex)
	{
		...
		if (m_debugDrawer->getDebugMode() & btIDebugDraw::DBG_DrawNormals)
		{
			...
			normal.normalize();
			btVector3 normalColor((btScalar)1, (btScalar)1, (btScalar)0);
			m_debugDrawer->drawLine(center, center + normal, normalColor);
		}
		m_debugDrawer->drawTriangle(wv0, wv1, wv2, m_color, (btScalar)1.0);
	}
};
...
void btCollisionWorld::debugDrawObject(const btTransform& worldTransform, const btCollisionShape* shape, const btVector3& color)
{
	...
	{
		getDebugDrawer()->drawTransform(worldTransform, (btScalar).1);
	}
			...
			default:
			{
				...
				if (shape->isPolyhedral())
				{
					...
					if (polyshape->getConvexPolyhedron())
					{
						...
						{
							btVector3 centroid((btScalar)0, (btScalar)0, (btScalar)0);
