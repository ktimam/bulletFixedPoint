...
public:
	...
	void setAnisotropicFriction(const btVector3& anisotropicFriction, int frictionMode = CF_ANISOTROPIC_FRICTION)
	{
		...
		bool isUnity = (anisotropicFriction[0] != (btScalar)1.f) || (anisotropicFriction[1] != (btScalar)1.f) || (anisotropicFriction[2] != (btScalar)1.f);
	...
	{
		bool fixed_base_multibody = (getActivationState() != FIXED_BASE_MULTI_BODY);
		bool island_sleeping = (getActivationState() != ISLAND_SLEEPING);
		bool disable_simulation = (getActivationState() != DISABLE_SIMULATION);
		return (fixed_base_multibody && island_sleeping && disable_simulation);
	}
	...
	void setContactStiffnessAndDamping(btScalar stiffness, btScalar damping)
	{
		...
		if (m_contactStiffness < (btScalar)SIMD_EPSILON)
		{
			m_contactStiffness = (btScalar)SIMD_EPSILON;
		}
	}
