...
void btConvexConvexAlgorithm ::processCollision(const btCollisionObjectWrapper* body0Wrap, const btCollisionObjectWrapper* body1Wrap, const btDispatcherInfo& dispatchInfo, btManifoldResult* resultOut)
{
	...
	if ((min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE) && (min1->getShapeType() == CAPSULE_SHAPE_PROXYTYPE))
	{
		...
		if (dist < threshold)
		{
			btAssert(normalOnB.length2() >= ((btScalar)SIMD_EPSILON * (btScalar)SIMD_EPSILON));
	...
	if ((min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE) && (min1->getShapeType() == SPHERE_SHAPE_PROXYTYPE))
	{
		...
		btScalar dist = capsuleCapsuleDistance(normalOnB, pointOnBWorld, capsuleA->getHalfHeight(), capsuleA->getRadius(),
											   (btScalar)0., capsuleB->getRadius(), capsuleA->getUpAxis(), 1,
											   body0Wrap->getWorldTransform(), body1Wrap->getWorldTransform(), threshold);
	...
	if ((min0->getShapeType() == SPHERE_SHAPE_PROXYTYPE) && (min1->getShapeType() == CAPSULE_SHAPE_PROXYTYPE))
	{
		...
		btScalar dist = capsuleCapsuleDistance(normalOnB, pointOnBWorld, (btScalar)0., capsuleA->getRadius(),
...

	{
		...
		if (min0->isPolyhedral() && min1->isPolyhedral())
		{
			...
			struct btWithoutMarginResult : public btDiscreteCollisionDetectorInterface::Result
			{
				...
				virtual void addContactPoint(const btVector3& normalOnBInWorld, const btVector3& pointInWorldOrg, btScalar depthOrg)
				{
					...
					if (m_reportedDistance < (btScalar)0.f)
					{
						m_foundResult = true;
					}
...
#else
						foundSepAxis = withoutMargin.m_foundResult && minDist < (btScalar)0;  //-(min0->getMargin()+min1->getMargin());
#endif
					}
				}
				if (foundSepAxis)
			...
			else
			{
				...
				if (dispatchInfo.m_enableSatConvex && polyhedronA->getConvexPolyhedron() && polyhedronB->getShapeType() == TRIANGLE_SHAPE_PROXYTYPE)
				{
					...
					if (0)
					{
...
#endif
						{
...

							{
								...
								btScalar planeEq = (btScalar)1e30f;
					...
					else
					{
						...
						if (dummy.m_hasContact && dummy.m_depth < (btScalar)0)
						{
							if (foundSepAxis)
							{
								if (dummy.m_normalOnBInWorld.dot(sepNormalWorldSpace) < (btScalar)0.99)
								{
									printf("?\n");
								}
							}
							...
							sepNormalWorldSpace.setValue((btScalar)0, (btScalar)0, (btScalar)1);  // = dummy.m_normalOnBInWorld;
		...
		if (m_numPerturbationIterations && resultOut->getPersistentManifold()->getNumContacts() < m_minimumPointsPerturbationThreshold)
		{
			...
			{
				sepNormalWorldSpace = gjkPairDetector.getCachedSeparatingAxis() * ((btScalar)1.f / l2);

				btPlaneSpace1(sepNormalWorldSpace, v0, v1);

				bool perturbeA = true;
				const btScalar angleLimit = (btScalar)0.125f * (btScalar)SIMD_PI;
